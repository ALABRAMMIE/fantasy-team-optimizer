...
team_size   = st.sidebar.number_input("Team Size",       value=default_ts, step=1)
...
max_usage_pct = st.sidebar.slider("Max Usage % per player/team",
    0,100,100,5, help="Cap fraction of teams any one player can appear on.")
...
def load_players(f):
    try:
        ...
        st.error(f"❌ Failed to read players file: {e}"); st.stop()
...
edited["base_FTPS"] = edited["FTPS"]  # snapshot raw FTPS
...
# --- Build one slider per bracket value ---
brackets = sorted([b for b in edited["Bracket"].dropna().unique()])
bracket_rand = {}
if brackets:
    st.sidebar.markdown("### FTPS Randomness by Bracket")
    for b in brackets:
        bracket_rand[b] = st.sidebar.slider(f"Bracket {b} Randomness %",
                                            0,100,0,5,
                                            key=f"rand_{b}")
# else: bracket_rand stays empty → no randomness
...
if st.sidebar.button("🚀 Optimize Teams"):
    ...
    # --- Maximize FTPS with per-bracket randomness ---
    ...
    # --- Closest FTP Match (unchanged) ---
        ...
        for _ in range(num_teams):
            ...
            for n in include_players:
                ...
                diffs=[(i,abs(p0["Value"]-target_values[i]))
