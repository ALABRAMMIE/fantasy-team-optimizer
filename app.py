...
def add_usage_constraints(prob, x_vars):
    if max_usage_pct < 100:
        for p in players:
            ...
            prob += (used_before + x_vars[name] <= max_usage_count, f"MaxUsage_{name}")
...
if st.sidebar.button("🚀 Optimize Teams"):
    ...
    # Maximize FTPS
        ...
        for idx in range(num_teams):
            # Special‐case for include override when only one team & zero randomness
            if idx == 0 and num_teams == 1 and ftps_rand_pct == 0 and include_players:
                ...
                prob0 += lpSum(x0[n] * next(p["Value"] for p in players if p["Name"] == n) for n in x0) <= budget
            ...
            # ALWAYS use original FTPS for the very first team (idx == 0)
            if idx == 0:
                ftps_values = {p["Name"]: p["FTPS"] for p in players}
            else:
                # apply ±rand only for idx > 0
                ftps_values = {
                    p["Name"]: p["FTPS"] * (1 + random.uniform(-ftps_rand_pct/100, ftps_rand_pct/100))
                    for p in players
                } if ftps_rand_pct > 0 else {p["Name"]: p["FTPS"] for p in players}
