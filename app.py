...
max_freq_pct = st.sidebar.number_input("Max Player Frequency (%)", min_value=0, max_value=100, value=100, step=5)
        ...
        if st.sidebar.button("🚀 Optimize Teams"):
            ...
            def add_bracket_constraints(prob,x_vars):
                if use_bracket_constraints and not bracket_fail:
                    ...
                    for p in players:
                        b=p.get("Bracket")
                        if b: brackets.setdefault(b,[]).append(x_vars[p["Name"]])
                    for lst in brackets.values(): prob += lpSum(lst)<=1
            ...
            if solver_mode in ["Maximize FTPS","Maximize Budget Usage"]:
                ...
                for _ in range(num_teams*100):
                    ...
                    for tgt in target_values[len(sel):]:
                        cands=sorted([p for p in avail if p["Name"] not in used and (p["Name"] in include_players or p["Name"] in exclude_players or frequency[p["Name"]]<max_occurrences)], key=lambda x:abs(x["Value"]-tgt))
                        if cands:
            ...
            st.download_button("📥 Download All Teams (Excel)", output, file_name="all_teams.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
