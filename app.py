...
if template_file:
    try:
        ...
        st.sidebar.warning("⚠️ Unable to read sheets.")
...
# default usage slider (for bracket-less players)
...
uploaded_file = st.sidebar.file_uploader("Players file", type=["xlsx"])
...
if "FTPS" not in edited: edited["FTPS"] = 0
...
# read target profile (closest-match)
target_values = None
...
def add_usage_constraints(prob, xvars):
    # skip usage constraints entirely if only building one team
    if num_teams <= 1:
        return
    for p in players:
        nm = p["Name"]
...
if st.sidebar.button("🚀 Optimize Teams"):
    ...
    # Maximize FTPS
        ...
        for idx in range(num_teams):
            ...
            prob += lpSum(x.values()) == team_size
            prob += lpSum(x[n]*next(q["Value"] for q in players if q["Name"]==n) 
                          for n in x) <= budget
            add_bracket_constraints(prob, x)
            add_usage_constraints(prob, x)
            for n in include_players: prob += x[n]==1
    ...
    # Closest FTP Match
        ...
        for _ in range(num_teams):
            ...
            for n in include_players:
                ...
                diffs = [(i,abs(p0["Value"]-target_values[i])) for i in range(team_size) 
                         if slots[i] is None]
