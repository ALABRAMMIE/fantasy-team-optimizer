...
def add_composition_constraints(prob, x):
    for b in brackets:
        ...
        if mn > 0:
            prob += lpSum(mem) >= mn, f"MinBracket_{b}"
        if mx < team_size:
            prob += lpSum(mem) <= mx, f"MaxBracket_{b}"
    ...
    # Maximize FTPS
        ...
        for idx in range(num_teams):
            if idx == 0:
                ...
                ftps_vals = {p['Name']: p['base_FTPS'] * (1 + random.uniform(-ftps_rand_pct/100, ftps_rand_pct/100)) for p in players}
        ...
        for _ in range(num_teams):
            ...
            for i in range(team_size):
                if slots[i] is not None:
                    continue
                tgt = target_values[i]
                cands = [p for p in players if p['Name'] not in used_names and p['Name'] not in exclude_players and (not use_bracket_constraints or p.get('Bracket') not in used_brackets)]
                if not cands:
                    st.error("🚫 Infeasible under those constraints.")
                    st.stop()
            ...
            if sum(p['Value'] for p in slots if p) > budget:
                st.error(f"❌ Budget exceeded ({sum(p['Value'] for p in slots if p):.2f} > {budget:.2f}).")
                st.stop()
