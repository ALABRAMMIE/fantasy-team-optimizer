...
# --- FTPS randomness ---
ftps_rand_pct = st.sidebar.slider("FTPS Randomness % for subsequent teams", 0, 100, 0, 5, help="± this percent noise on FTPS for teams 2…N")

# === Rank TIERS controls ===
st.sidebar.markdown("### Rank TIERS (Rank Buckets)")
use_tiers = st.sidebar.checkbox("Use Rank Tiers (shuffle FTPS within rank buckets)", value=False)
tiers_apply_only_after_first = st.sidebar.checkbox("Apply rank-tier shuffling for teams 2…N only", value=True)
tiers_text_default = "1-5\n6-10\n11-15\n16-20\n21-25\n26-30"
tiers_text = st.sidebar.text_area("Tier ranges (one per line, e.g., 1-5)", value=tiers_text_default)
...
allow_draws = st.sidebar.checkbox("Allow Draw outcomes", value=(sport in ["Football","Soccer","American Football"]))
...
fixed_win_ftps  = st.sidebar.number_input("Fixed FTPS for WIN", 0.0, 1000.0, 10.0, 0.5, disabled=(outcome_value_mode!="Use Fixed FTPS values"))
fixed_draw_ftps = st.sidebar.number_input("Fixed FTPS for DRAW", 0.0, 1000.0, 6.0, 0.5, disabled=(outcome_value_mode!="Use Fixed FTPS values" or not allow_draws))
fixed_loss_ftps = st.sidebar.number_input("Fixed FTPS for LOSS", 0.0, 1000.0, 2.0, 0.5, disabled=(outcome_value_mode!="Use Fixed FTPS values"))
...
outcome_apply_only_after_first = st.sidebar.checkbox("Apply outcome randomness for teams 2…N only", value=False)

# NEW: bracket coupling toggle
bracket_linked_outcomes = st.sidebar.checkbox(
    "Bracket-linked outcomes (1v1 coupling)", value=True,
    help="If a player in a bracket is Tier 1 (always win), opponents in that bracket are forced to lose. Otherwise, exactly one side wins (draw only when all are Tier 3 and draws are allowed)."
)

# --- Rank-1 controls (optional) ---
always_include_rank1_team1 = st.sidebar.checkbox("Always include Rank 1 in Team 1", value=True)
min_usage_rank1_pct = st.sidebar.slider("Min usage % for Rank 1 (across all teams)", 0, 100, 0, 5)
...
def ensure_rank_column(dframe):
    if "Rank" not in dframe.columns:
        out=dframe.copy(); out["Rank"]=None
    else:
        out=dframe.copy(); out["Rank"]=pd.to_numeric(out["Rank"], errors="coerce")
    tmp=out.copy()
    ...
    ranks=[]; used=set(int(r) for r in provided if r>0); next_seq=1
...
if "FTPS" not in edited.columns: edited["FTPS"]=0.0
...
if use_outcome_tiers:
    ...
    if "OutcomeTier" not in edited.columns: edited["OutcomeTier"]=None
...
def add_composition_constraints(prob, x):
    for b in brackets:
        ...
        members=[x[p["Name"]] for p in players if p.get("Bracket")==b]
        if mn>0: prob += lpSum(members) >= mn, f"MinBracket_{b}"
        if mx<team_size: prob += lpSum(members) <= mx, f"MaxBracket_{b}"
...
def independent_sample_outcome(tier:int):
    ...
    # tier 3
    pW=max(0,min(1,t3_p_win)); pD=0.0 if not allow_draws else max(0,min(1-pW,t3_p_draw))
    r=random.random()
    if r<pW: return 'W'
    if r<pW+pD: return 'D'
    return 'L'
...
def sample_outcomes_by_bracket():
    ...
    Rules:
      - If exactly one Tier-1 in a bracket => that one 'W', others 'L'.
      - If multiple Tier-1 in a bracket => pick winner by highest base_FTPS (warn), others 'L'.
      - If no Tier-1:
          * If ALL players are Tier-3 and draws allowed -> with prob t3_p_draw: all 'D'; else one 'W' (weighted by t3_p_win), rest 'L'.
          * Otherwise (any Tier-2 present) -> no draw; one 'W' (weighted by per-player Pwin), rest 'L'.
    If a player has no bracket or is in a singleton bracket, falls back to independent sampling.
    """
    # group by bracket value (skip NaN)
    groups=defaultdict(list)
    for p in players:
        br=p.get("Bracket")
        if pd.notna(br): groups[str(br)].append(p)
    outcomes={}

    # handle bracketed players
    for br, group in groups.items():
        ...
        # 2+ players: apply coupling
        tiers=[int(p.get("OutcomeTier",3)) for p in group]
        tier1_ix=[i for i,t in enumerate(tiers) if t==1]

        if len(tier1_ix)>=1:
            # winner = the sole Tier-1; if multiple, choose highest base_FTPS
            if len(tier1_ix)>1:
                st.warning(f"Bracket '{br}': multiple Tier-1 detected; choosing winner by highest base_FTPS.")
                winner = max((group[i] for i in tier1_ix), key=lambda r: r.get("base_FTPS", r.get("FTPS",0.0)))
            else:
        ...
        # draw branch (only if all Tier-3)
        if can_draw:
            if random.random() < max(0,min(1,t3_p_draw)):
                for p in group: outcomes[p["Name"]]='D'
                continue
        # choose exactly one winner, weighted by Pwin
        weights=[]
        for p,t in zip(group, tiers):
            if t==2: w=max(0.0,min(1.0,t2_p_win))
            else:    w=max(0.0,min(1.0,t3_p_win))
            weights.append(w)
        total=sum(weights)
        if total<=0:
            # fallback: uniform
    ...
    # players without bracket value (or not in groups)
    bracketed_names=set(outcomes.keys())
    for p in players:
        if p["Name"] in bracketed_names: continue
        outcomes[p["Name"]]=independent_sample_outcome(int(p.get("OutcomeTier",3)))
...
def build_ftps_values_for_team(team_index:int):
    ...
    apply_outcomes_now = use_outcome_tiers and ((team_index==0 and not outcome_apply_only_after_first) or (team_index>0))
    if apply_outcomes_now:
        if bracket_linked_outcomes and "Bracket" in edited.columns:
            ...
            for p in players:
                sampled_outcomes[p["Name"]] = independent_sample_outcome(int(p.get("OutcomeTier",3)))
    ...
    # Rank-tier shuffling (after outcomes)
    ...
    if apply_rank_tiers_now:
        ...
        for p in players:
            t=p.get("Tier")
            if t: tier_to_players[t].append(p["Name"])
        for t,names in tier_to_players.items():
            if len(names)>=2:
                vals=[ftps_vals[n] for n in names]
                random.shuffle(vals)
                for n,v in zip(names, vals): ftps_vals[n]=v
...
if st.sidebar.button("🚀 Optimize Teams"):
    ...
    def get_rank1_name(ftps_vals=None):
        ...
        if cand: return cand[0]["Name"]
        if ftps_vals: return max(ftps_vals.items(), key=lambda kv: kv[1])[0]
    ...
    if solver_mode=="Maximize Budget Usage":
        ...
        for _ in range(num_teams):
            ...
            for n in include_players:
                ...
                slots[best_i]=p0; used_names.add(n)
                if use_bracket_constraints and p0.get("Bracket"): used_brackets.add(p0["Bracket"])
            for i in range(team_size):
                if slots[i] is not None: continue
                tgt=target_values[i]; cands=[]
                for p in players:
                    if p["Name"] in used_names or p["Name"] in exclude_players: continue
                    if use_bracket_constraints and p.get("Bracket") in used_brackets: continue
                    used=sum(1 for prev in prev_sets if p["Name"] in prev)
                    if p["Name"] not in include_players and used>=cap: continue
                    cands.append(p)
                if not cands: st.error("🚫 Infeasible under those constraints."); st.stop()
            ...
            if cost>budget: st.error(f"❌ Budget exceeded ({cost:.2f} > {budget:.2f})."); st.stop()
            ...
            for p in slots:
                if p:
                    ...
                    if use_outcome_tiers: row["OutcomeTier"]=p.get("OutcomeTier",3)
                    row["Tier"]=p.get("Tier")
                    team.append(row)
            all_teams.append(team); prev_sets.append(current)
    ...
    for idx,team in enumerate(all_teams, start=1):
        df_t=pd.DataFrame(team); df_t["Team"]=idx
        df_t["Selectie (%)"]=df_t["Name"].apply(lambda n: round(
            sum(1 for t in all_teams if any(p["Name"]==n for p in t)) / len(all_teams) * 100, 1))
        merged.append(df_t)
    ...
    buf.seek(0)
    st.download_button("📥 Download All Teams (Excel)", buf, file_name="all_teams_v2.xlsx",
