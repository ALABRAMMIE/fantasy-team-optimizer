...
def add_bracket_constraints(prob, x):
    if use_bracket_constraints:
        for b in brackets:
            prob += lpSum(x[p['Name']] for p in players if p.get('Bracket')==b) <= 1, f"UniqueBracket_{b}"
...
if st.sidebar.button("🚀 Optimize Teams"):
    ...
    if solver_mode=="Maximize Budget Usage":
        ...
        for _ in range(num_teams):
            ...
            all_teams.append(team); prev_sets.append({p['Name'] for p in team}); upper=sum(p['Value'] for p in team)-0.001
        # Tour Substitutes
    ...
    # Maximize FTPS
        ...
        for _ in range(num_teams):
            slots=[None]*team_size; br_s=set(); names_s=set()
            for n in include_players:
                ...
                idx_min=min(diffs,key=lambda x:x[1])[0]; slots[idx_min]=p0; names_s.add(n)
                if use_bracket_constraints and p0.get('Bracket'): br_s.add(p0['Bracket'])
            for i in range(team_size):
                if slots[i] is not None: continue
                tgt=target_values[i]
                cands=[p for p in players if p['Name'] not in names_s and p['Name'] not in exclude_players and (not use_bracket_constraints or p.get('Bracket') not in br_s)]
                if not cands: st.error("🚫 Infeasible under those constraints."); st.stop()
                pick=min(cands,key=lambda p:abs(p['Value']-tgt)); slots[i]=pick; names_s.add(pick['Name'])
                if use_bracket_constraints and pick.get('Bracket'): br_s.add(pick['Bracket'])
            if sum(p['Value'] for p in slots if p)>budget: st.error(f"❌ Budget exceeded ({sum(p['Value'] for p in slots if p):.2f} > {budget:.2f})."); st.stop()
    ...
    if tour_mode and sport=='Cycling':
        ...
        styled=subs_df.style.apply(lambda _:['background-color: lightyellow']*len(subs_df),axis=1)
        st.dataframe(styled)
    # Download
    buf=BytesIO()
    with pd.ExcelWriter(buf,engine='openpyxl') as writer:
        for idx,team in enumerate(all_teams, start=1):
            df_t=pd.DataFrame(team)
            df_t["Selectie (%)"]=df_t["Name"].apply(lambda n:round(sum(1 for t in all_teams if any(p['Name']==n for p in t))/len(all_teams)*100,1))
            df_t.to_excel(writer,sheet_name=f"Team{idx}",index=False)
    buf.seek(0)
