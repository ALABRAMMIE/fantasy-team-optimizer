        ...
        if st.sidebar.button("🚀 Optimize Teams"):
            ...
            if solver_mode in ["Maximize FTPS", "Maximize Budget Usage"]:
                ...
                for _ in range(num_teams):
                    ...
                    for n in sel: frequency[n] += 1
                    all_teams.append([p for p in players if p["Name"] in sel])
                    # Fallback to fill remainder if solver picks fewer than team_size
                    if len(sel) < team_size:
                        remaining = [p for p in players if p['Name'] not in sel and p['Name'] not in exclude_players and (p['Name'] in include_players or frequency[p['Name']] < max_occurrences)]
                        if solver_mode == 'Maximize Budget Usage':
                            remaining.sort(key=lambda p: -p['Value'])
                        else:
                            remaining.sort(key=lambda p: -p.get('FTPS', 0))
                        for p in remaining:
                            if len(sel) >= team_size:
                                break
                            sel.append(p['Name'])
                            frequency[p['Name']] += 1
                        all_teams[-1] = [p for p in players if p['Name'] in sel]
            ...
            # Generate greedy teams
                ...
                for _ in range(attempts):
                    ...
                    # greedy fill with fallback
                    for tgt in target_values[len(sel):]:
                        # primary filter
                        cands = [
                            p for p in avail
                            if p["Name"] not in used and
                            (p["Name"] in include_players or p["Name"] in exclude_players or frequency[p["Name"]] < max_occurrences) and
                            (not use_bracket_constraints or p.get("Bracket") not in br_used)
                        ]
                        # if none, relax bracket constraint
                        if not cands:
                            cands = [
                                p for p in avail
                                if p["Name"] not in used and
                                (p["Name"] in include_players or p["Name"] in exclude_players or frequency[p["Name"]] < max_occurrences)
                            ]
                        # if still none, relax frequency constraint
                        if not cands:
