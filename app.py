...
if st.sidebar.button("🚀 Optimize Teams"):
    ...
    for idx in range(1,len(all_teams)+1):
        with st.expander(f"Team {idx}"):
            ...
            df_t['Selectie (%)']=df_t['Name'].apply(lambda n:round(sum(1 for r in output_records if r['Name']==n and r['Role']=='Main')/len(all_teams)*100,1))
            st.dataframe(df_t)
    # Display Substitutes
    if subs:
        with st.expander("🔄 Tour Substitutes"):
            df_s=pd.DataFrame([r for r in output_records if r['Role']=='Substitute'])
            df_s['Selectie (%)']=df_s['Name'].apply(lambda n:round(sum(1 for r in output_records if r['Name']==n and r['Role']=='Main')/len(all_teams)*100,1))
